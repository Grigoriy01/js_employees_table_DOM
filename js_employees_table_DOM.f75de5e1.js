...
tbodyContainer.addEventListener('click', (checkedRow)=>{
    ...
    if (currActive || !targetRow) currActive.classList.remove('active');
...
headers.addEventListener('click', (checked)=>{
    ...
    function getSortList(data, key) {
        ...
        const selectedTools = typeof data[0][key] === 'number' ? sortNumber : sortString;
...
myForm.addEventListener('submit', (e)=>{
    ...
    if (inputName.length < 4 || inputName === '') return notification(error, colName);
    if (+inputAge < 18 || +inputAge > 90 || inputAge === '') return notification(error, age);
    if (inputPos === '') return notification(error, position);
